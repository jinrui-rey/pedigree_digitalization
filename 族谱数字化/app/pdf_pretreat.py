...
def horizontal_projection(binary_image: np.array,
                          trim: bool = False) -> np.array:
...
def vertical_projection(binary_image: np.array, trim: bool = False) -> np.array:
    result = np.zeros_like(binary_image)
...
def reject_small_regions(coordinate):
    for i in range(len(coordinate)):
        x1, y1, x2, y2 = coordinate[i]
        side_x = abs(x2 - x1)
        side_y = abs(y2 - y1)
        ...
        if area < 4700:  # 估计值，需要随时改变
            coordinate[i] = [0, 0, 0, 0]


def cut_title(bimg, img):
